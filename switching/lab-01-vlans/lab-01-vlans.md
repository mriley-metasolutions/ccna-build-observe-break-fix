...
A switch straight out of the box will put every port in the same broadcast domain. Every device can reach every other device, and every broadcast reaches everyone. That is fine for a handful of machines, but it does not hold up in a real organization where you want the sales team separated from engineering, or guest traffic walled off from the servers. VLANs are how you carve one physical switch into several logical ones, and they are where the switching half of the CCNA really begins.
